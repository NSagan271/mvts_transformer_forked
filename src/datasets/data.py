...
class BatVocalizationData(BaseData):
    """
    Dataset class for Bat Vocalization dataset, reading .mat files generated by ../../CollectChirpSequences.ipynb.
    (11/06/2023)
    ...
    def load_all(self, root_dir, nfft, fft_stride, file_list=None, pattern=None):
        ...
        Args:
            root_dir: directory containing all individual .mat files
            file_list: optionally, provide a list of file paths within `root_dir` to consider.
                Otherwise, entire `root_dir` contents will be used.
            pattern: optionally, apply regex string to select subset of files
        Returns:
        ...
        if len(input_paths) == 0:
            raise Exception("No .mat files found using pattern: '{}'".format(pattern))
    ...
    def load_single_file(filepath):
        with h5py.File(filepath, 'r') as f:
            return [f[h5py.h5r.get_name(elem, f.id)][:] for elem in f['chirp_sequence_array']]

data_factory = {'weld': WeldData,
                'tsra': TSRegressionArchive,
                'pmu': PMUData,
                'bat': BatVocalizationData}
